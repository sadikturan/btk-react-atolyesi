...
export const updateCartItemDb = createAsyncThunk(
  ...
  async ({ id, quantity }, { rejectWithValue }) => {
    try {
      const data = await update_cart_item(id, quantity);
      return data.items;
    } catch (error) {
      return rejectWithValue(
        error.response?.data || error.message || "Fetch cart failed"
...
export const deleteCartItemDb = createAsyncThunk(
  "cart/deleteCartItemDb",
  async (id, { rejectWithValue }) => {
    try {
      const data = await delete_cart_item(id);
      return data.items;
    } catch (error) {
      return rejectWithValue(
        error.response?.data || error.message || "Delete item failed"
      );
    }
  }
);
...
export const clearCartDb = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const data = await clear_cart();
      return data.items;
    } catch (error) {
      return rejectWithValue(
        error.response?.data || error.message || "Clear cart failed"
      );
    }
  }
);
...
const cartSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteCartItemDb.rejected, (state, action) => {
        state.loading = false;
        console.error("Update failed:", action.payload);
      })
