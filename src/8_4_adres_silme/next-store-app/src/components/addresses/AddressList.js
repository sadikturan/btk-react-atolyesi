...
import { delete_address, get_addresses } from "@/lib/api";
...
export default function AddressList() {
  ...
  const handleModalSuccess = (newAddress) => {
  ...
  const handleDelete = async (address_id) => {
    ...
    try {
      ...
      setAddressList(addressList.filter((i) => i.id !== address_id));
      if (selected == address_id) setSelected(null);
    } catch (err) {
            ...
            >
              ...
              >
                <div className="flex justify-between">
                  ...
                  {address.is_default && (
                    <span className="absolute bottom-2 right-2 text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded">
                      Varsayılan
                    </span>
                  )}
                </div>
